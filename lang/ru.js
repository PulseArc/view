export default {
    ...
    notice_none: 'У вас ещё нет никаких уведомлений, зарегистрируйтесь на сайте <b>{site}</b>, чтобы следить за новыми сериями и релизами.',
    notice_in_quality: 'В качестве',
    notice_none_account: 'У вас ещё нет никаких уведомлений, добавьте сериалы в закладки и ожидайте уведомления о новых сериях.',
    ...
    https_text: 'Вы используйте протокол HTTPS, c которым Cinematic работает некорректно. Для корректной работы приложения используйте адрес с протоколом HTTP',
    ...
    termsofuse_t_02: "Cinematic — это платформа, предназначенная для ознакомления пользователей с актуальной информацией киноиндустрии, включая фильмы, сериалы, мультфильмы и сопутствующие данные. Источником информации является открытый ресурс — веб-сайт themoviedb.org.",
    termsofuse_t_03: "Контент и источники",
    termsofuse_t_04: "Cinematic использует API (Application Programming Interface) от каталога themoviedb.org для получения данных о фильмах и телесериалах, а также для отображения связанных медиафайлов, таких как постеры, трейлеры и другие материалы.",
    termsofuse_t_05: "Доступ к базе данных: API от themoviedb.org предоставляет приложению Cinematic доступ к базе данных фильмов и телесериалов, обеспечивая возможность поиска медиафайлов и просмотра детальной информации о них.",
    termsofuse_t_06: "Техническое взаимодействие между приложением Cinematic и themoviedb.org осуществляется через запросы к API, которые отправляются с клиента на серверы themoviedb.org для получения данных. Themoviedb.org обрабатывает запросы и передает запрошенные данные обратно в приложение Cinematic.",
    termsofuse_t_07: "Таким образом, благодаря использованию API от themoviedb.org, приложение Cinematic предоставляет пользователям доступ к обширной базе данных контента, соблюдая при этом условия использования и ограничения, установленные правообладателем.",
    termsofuse_t_08: "Расширения (Extensions)",
    termsofuse_t_09: "Расширения — это мощный инструментарий, доступный для приложения Cinematic, который позволяет пользователям расширить его функционал, добавить дополнительные возможности и адаптировать контент под свои индивидуальные потребности.",
    termsofuse_t_10: "Отказ от ответственности: Пользователи устанавливают и используют расширения исключительно на свой страх и риск. Все последствия, возникающие в результате использования сторонних расширений, возлагаются на пользователя. Рекомендуется тщательно оценивать их надежность и безопасность.",
    termsofuse_t_11: "Уведомление о нарушениях авторских прав",
    termsofuse_t_12: "Если вы обнаружили предполагаемое нарушение авторских прав, просим связаться с разработчиками через раздел «Контакты». Важно учитывать, что приложение Cinematic использует API от каталога themoviedb.org, который предоставляет открытый доступ к общедоступной информации о фильмах и сериалах, и, следовательно, не нарушает авторских прав напрямую.",
    termsofuse_t_13: "Порядок устранения: Если вы считаете, что контент в Cinematic нарушает права, обратитесь к правообладателю этого контента с требованием удалить его с сайта themoviedb.org. Приложение Cinematic удалит отображение контента после получения соответствующего официального уведомления о нарушении авторских прав.",

    install_app_apk_text: 'Мы заметили, что для запуска Cinematic на Android вы используете MediaStationX. Мы рекомендуем вам установить Lampa в виде APK-приложения. Вы можете скачать последнюю версию приложения здесь:',
    ...
    loading_progress_step_0: 'Инициализация',
    loading_progress_step_1: 'Загрузка локализации',
    loading_progress_step_2: 'Верификация источника',
    loading_progress_step_3: 'Загрузка расширений',
    loading_progress_step_4: 'Завершение',
    loading_progress_step_5: 'Система готова',

    install_extension_rule_1: 'Установка стороннего плагина',
    install_extension_rule_2: 'Плагины создаются независимыми разработчиками, и Cinematic не проверяет их содержимое. Устанавливая их, вы соглашаетесь с тем, что:',
    ...
    install_extension_rule_6: 'Cinematic не поддерживает и не одобряет плагины, связанные с пиратским контентом.',
